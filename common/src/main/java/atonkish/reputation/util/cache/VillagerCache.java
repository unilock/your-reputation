...
public class VillagerCache {
    private static final Map<PlayerEntity, Cache<VillagerEntity, Data>> VILLAGER_CACHE_MAP = new HashMap<>();

    public static Cache<VillagerEntity, Data> getOrCreate(PlayerEntity player) {
        if (!VillagerCache.VILLAGER_CACHE_MAP.containsKey(player)) {
            Cache<VillagerEntity, Data> cache = CacheBuilder
                    .newBuilder()
