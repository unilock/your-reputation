...
public class IronGolemCache {
    private static final Map<PlayerEntity, Cache<IronGolemEntity, Data>> IRON_GOLEM_CACHE_MAP = new HashMap<>();

    public static Cache<IronGolemEntity, Data> getOrCreate(PlayerEntity player) {
        if (!IronGolemCache.IRON_GOLEM_CACHE_MAP.containsKey(player)) {
            Cache<IronGolemEntity, Data> cache = CacheBuilder
                    .newBuilder()
